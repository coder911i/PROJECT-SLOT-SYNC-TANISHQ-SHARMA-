...
app.post('/api/schedule', (req, res) => {
  ...
  db.prepare(`INSERT INTO sessions (id, candidate_name, candidate_availability, status) VALUES (?,?,?,'processing')`)
    .run(sessionId, candidateName || 'Candidate', candidateAvailability);
  interviewers.forEach(iv => {
    db.prepare(`INSERT INTO interviewers (id, session_id, name, availability) VALUES (?,?,?,?)`)
      .run(uuidv4(), sessionId, iv.name, iv.availability);
  });
  const result = findBestSlots(candidateAvailability, interviewers);
  db.prepare(`INSERT INTO results (id, session_id, slots_json, conflicts_json) VALUES (?,?,?,?)`)
